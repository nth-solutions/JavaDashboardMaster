...
public class PendulumSpreadsheetController {
    ...
    private String workbookPath = "C:\\Users\\60000587.CASDSCHOOLS\\Documents\\GitHub\\JavaDashboardMaster\\Pendulum Template REV-Q3.xlsx";
        ...
        for (int axis = 1; axis < 10; axis++) {
            if (dataSamples != null) {
                ...
                for (int columnIndex = 0; columnIndex < ColumnData.size(); columnIndex++) { //Loops through every data point in the List
                    ...
                    workbook.getWorksheets().get(0).getCells().get(columnIndex + rowOffset).setValue(ColumnData.get(columnIndex));  //Adds the data held at columnIndex to each row of the data
                }
            }
        }
